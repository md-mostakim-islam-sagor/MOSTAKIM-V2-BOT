...
const LIVELOG_FILE = path.join(ROOT, 'MAIN/bot-livelog.json');
const BROADCAST_QUEUE = path.join(ROOT, 'MAIN/bot-broadcast-queue.json');

router.get('/api/livelog', requireAdmin, (req, res) => {
    let buf = [];
    try { buf = JSON.parse(fs.readFileSync(LIVELOG_FILE, 'utf-8')); } catch {}
    const filter = (req.query.level || 'ALL').toUpperCase();
    const lines = filter === 'ALL' ? buf : buf.filter(line => line.includes(`[${filter}]`));
    res.json({ lines, total: buf.length, filtered: lines.length });
});

router.delete('/api/livelog', requireAdmin, (req, res) => {
    try { fs.writeFileSync(LIVELOG_FILE, '[]'); } catch {}
    res.json({ success: true });
});

router.get('/api/broadcast/status', requireAdmin, (req, res) => {
    try {
        const live = getLiveBotStats();
        const threads = (live && live.threads) || [];
        let queue = [];
        try { queue = JSON.parse(fs.readFileSync(BROADCAST_QUEUE, 'utf-8')); } catch {}
        res.json({ threadCount: threads.length, queue });
    } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post('/api/broadcast', requireAdmin, (req, res) => {
    try {
        const { message, mode, delay } = req.body;
        if (!message || !message.trim()) return res.status(400).json({ error: 'Message is required' });
        const live = getLiveBotStats();
        const threadIDs = (live && live.threadIDs && live.threadIDs.length)
            ? live.threadIDs
            : (live && live.threads || []).map(t => t.id || t).filter(Boolean);
        if (!threadIDs.length) return res.status(400).json({ error: 'No active threads found. Make sure the bot is online.' });
        let queue = [];
        try { queue = JSON.parse(fs.readFileSync(BROADCAST_QUEUE, 'utf-8')); } catch {}
        const job = {
            id: Date.now(),
            ...
            threads: threadIDs,
            delay: Math.max(500, parseInt(delay) || 1000),
            createdAt: Date.now(),
            status: 'pending',
            sent: 0,
            failed: 0,
        ...
        queue.push(job);
        fs.writeFileSync(BROADCAST_QUEUE, JSON.stringify(queue, null, 2));
        res.json({ success: true, jobId: job.id, queuedTo: job.threads.length });
    } catch (e) { res.status(500).json({ error: e.message }); }
});

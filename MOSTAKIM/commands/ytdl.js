...
async function downloadYouTubeViaAPI(url, outPath, audioOnly = false) {
  // Use mostakim-media-downloaders (ytdown) — different endpoint, returns audio field
  try {
    const { ytdown } = require("mostakim-media-downloaders");
    const result = await ytdown(url);
    if (!result?.status || !result?.data) throw new Error("ytdown no data");
    const mediaUrl = audioOnly
      ? (result.data.audio || result.data.video)
      : (result.data.video_hd || result.data.video || result.data.audio);
    if (!mediaUrl) throw new Error("ytdown no URL");
    const vr = await axios.get(mediaUrl, { responseType: "arraybuffer", timeout: 90000, headers: { "User-Agent": "Mozilla/5.0" } });
    fs.writeFileSync(outPath, Buffer.from(vr.data));
    const dur = result.data.quality ? `${Math.floor(result.data.quality / 60)}:${String(result.data.quality % 60).padStart(2,"0")}` : "";
    return { title: result.data.title || "Video", uploader: result.data.channel || "", durationStr: dur };
  } catch {}

  // Last resort: direct nayan API — video only (no audio stream)
  // Refuse to use this for audio-only mode — it would produce corrupt mp3
  if (audioOnly) throw new Error("Audio download ব্যর্থ। YouTube audio URL পাওয়া যায়নি।");
  ...
  const d = res.data;
  if (!d.status || !d.data) throw new Error("nayan API no data");
  const videoUrl = d.data.high || d.data.low;
  if (!videoUrl) throw new Error("nayan API no URL");
...
module.exports.run = async function ({ api, event, args }) {
  ...
  try {
    ...
    try {
      ...
      info = await downloadYouTubeViaAPI(url, outPath, audioOnly);
    }

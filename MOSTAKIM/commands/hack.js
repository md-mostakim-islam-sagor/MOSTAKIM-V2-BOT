...
try { ({ loadImage, createCanvas, registerFont } = require("canvas")); } catch (_) {}

// Register the bundled Arial-compatible font once on load
try {
  if (registerFont) registerFont(__dirname + "/cache/SVN-Arial 2.ttf", { family: "SVN-Arial" });
} catch (_) {}
...
module.exports.run = async function ({ api, event, args }) {
  try {
    ...
    const bgURL = "https://i.ibb.co.com/W4H0tgh5/VQXVi-KI.png";
    ...
    ctx.font = "400 23px SVN-Arial";
    ctx.fillStyle = "#1878F3";

    const lines = await wrapText(ctx, name, 1160);
    const lineHeight = 28;
    lines.forEach((line, i) => ctx.fillText(line, 200, 497 + i * lineHeight));

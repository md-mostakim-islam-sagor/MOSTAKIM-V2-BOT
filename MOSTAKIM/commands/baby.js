...
const apiList = "https://raw.githubusercontent.com/md-mostakim-islam-sagor/mostakim-api-hub/refs/heads/main/mostakim-api.json";
...
async function getMainAPI() {
  ...
  // API base only — new API uses /baby/* routes on this domain
  _cachedSimsimi = res.data.simsimi;
  _cacheTime     = Date.now();
  return _cachedSimsimi;
}

const ADMIN_KEY = process.env.BABY_ADMIN_KEY || "";

// ── New "baby-sim-v1" API response shape helper ─────────────────────
// { status, data: { choices: [ { message: {...} } ] } }
function unwrap(res) {
  const msg = res?.data?.data?.choices?.[0]?.message;
  return msg || {};
}
...
function apiErrorText(res) {
  ...
  return msg.error || msg.text || "Unknown API error";
}
...
module.exports.run = async function ({ api, event, args, Users }) {
  try {
    ...
    if (["remove", "rm"].includes(command)) {
      if (!ADMIN_KEY) return sendPlain(api, event.threadID, "❌ Admin key configured নেই। BABY_ADMIN_KEY secret সেট করো।", event.messageID, uid);
      ...
      // Find the answer's index for this question, then remove that index
      const listRes = await fetchWithRetry(`${simsim}/baby?list=${encodeURIComponent(ask)}`);
      const listMsg = unwrap(listRes);
      const idx = Array.isArray(listMsg.answers) ? listMsg.answers.indexOf(ans) : -1;

      const removeUrl = idx >= 0
        ? `${simsim}/baby?remove=${encodeURIComponent(ask)}&index=${idx}&key=${encodeURIComponent(ADMIN_KEY)}`
        : `${simsim}/baby?remove=${encodeURIComponent(ask)}&key=${encodeURIComponent(ADMIN_KEY)}`;
      const res = await fetchWithRetry(removeUrl);
      if (isApiError(res)) return sendPlain(api, event.threadID, `❌ ${apiErrorText(res)}`, event.messageID, uid);
      return sendPlain(api, event.threadID, unwrap(res).text || "Removed!", event.messageID, uid);
    }

    if (command === "list") {
      const res = await fetchWithRetry(`${simsim}/baby/count`);
      const msg = unwrap(res);
      if (!isApiError(res)) {
        return sendPlain(api, event.threadID,
          `♾ Total Questions: ${msg.totalAsk}\n★ Total Replies: ${msg.totalAnswer}\n🔥 Total Hits: ${msg.totalMessageHits}\nDeveloper: MOSTAKIM`,
          event.messageID, uid);
      }
      return sendPlain(api, event.threadID, `❌ ${apiErrorText(res)}`, event.messageID, uid);
    }
    ...
    if (command === "edit") {
      ...
      const [ask, , newReply] = parts.map(p => p.trim());
      const res = await fetchWithRetry(`${simsim}/baby?edit=${encodeURIComponent(ask)}&replace=${encodeURIComponent(newReply)}&key=${encodeURIComponent(ADMIN_KEY)}`);
      if (isApiError(res)) return sendPlain(api, event.threadID, `❌ ${apiErrorText(res)}`, event.messageID, uid);
      return sendPlain(api, event.threadID, unwrap(res).text || "Updated!", event.messageID, uid);
    }
    ...
    if (command === "teach") {
      ...
      const [ask, ans] = parts.map(p => p.trim());
      const teachUrl = `${simsim}/baby?teach=${encodeURIComponent(ask)}&reply=${encodeURIComponent(ans)}&teacher=${encodeURIComponent(senderName)}`;
      const res = await fetchWithRetry(teachUrl);
      if (isApiError(res)) return sendPlain(api, event.threadID, `❌ ${apiErrorText(res)}`, event.messageID, uid);
      return sendPlain(api, event.threadID, unwrap(res).text || "Taught!", event.messageID, uid);
    }

    const res  = await fetchWithRetry(`${simsim}/baby?text=${encodeURIComponent(query)}&sender=${encodeURIComponent(senderName)}`);
    if (isApiError(res)) return sendPlain(api, event.threadID, `❌ ${apiErrorText(res)}`, event.messageID, uid, query, "");
    const pick = unwrap(res).reply || "...";
    // API's reply already personalizes with "@sender ..." — don't double-prefix the name.
    await sendPlain(api, event.threadID, pick, event.messageID, uid, query, pick);
  ...
  try {
    ...
    if (isAdmin(uid)) {
      ...
      if (/^\.teach\s+/i.test(body) || /^teach:\s*/i.test(body)) {
        ...
        const teachUrl = `${simsim}/baby?teach=${encodeURIComponent(originalQuery)}&reply=${encodeURIComponent(newAns)}&teacher=${encodeURIComponent(senderName)}`;
        const res = await fetchWithRetry(teachUrl);
        if (isApiError(res)) return api.sendMessage(`❌ ${apiErrorText(res)}`, threadID, () => {}, messageID);
        return api.sendMessage(
          `✅ 𝗧𝗘𝗔𝗖𝗛 𝗦𝗨𝗖𝗖𝗘𝗦𝗦\n` +
          `━━━━━━━━━━━━━━\n` +
      ...
      if (/^\.wrong$/i.test(body) || /^wrong$/i.test(body)) {
        if (!ADMIN_KEY) return api.sendMessage("❌ Admin key configured নেই। BABY_ADMIN_KEY secret সেট করো।", threadID, () => {}, messageID);
        ...
        const listRes = await fetchWithRetry(`${simsim}/baby?list=${encodeURIComponent(originalQuery)}`);
        const listMsg = unwrap(listRes);
        const idx = Array.isArray(listMsg.answers) ? listMsg.answers.indexOf(botReply) : -1;
        const removeUrl = idx >= 0
          ? `${simsim}/baby?remove=${encodeURIComponent(originalQuery)}&index=${idx}&key=${encodeURIComponent(ADMIN_KEY)}`
          : `${simsim}/baby?remove=${encodeURIComponent(originalQuery)}&key=${encodeURIComponent(ADMIN_KEY)}`;
        const res = await fetchWithRetry(removeUrl);
        if (isApiError(res)) return api.sendMessage(`❌ ${apiErrorText(res)}`, threadID, () => {}, messageID);
        return api.sendMessage(
      ...
      if (/^\.edit\s+/i.test(body) || /^edit:\s*/i.test(body)) {
        ...
        const res = await fetchWithRetry(
          `${simsim}/baby?edit=${encodeURIComponent(originalQuery)}&replace=${encodeURIComponent(newAns)}&key=${encodeURIComponent(ADMIN_KEY)}`
        );
        if (isApiError(res)) return api.sendMessage(`❌ ${apiErrorText(res)}`, threadID, () => {}, messageID);
        return api.sendMessage(
          `✏️ 𝗘𝗗𝗜𝗧 𝗦𝗨𝗖𝗖𝗘𝗦𝗦\n` +
    ...
    const res  = await fetchWithRetry(`${simsim}/baby?text=${encodeURIComponent(replyText)}&sender=${encodeURIComponent(senderName)}`);
    if (isApiError(res)) return;
    const pick = unwrap(res).reply || "...";
    // API's reply already personalizes with "@sender ..." — don't double-prefix the name.
    await sendPlain(api, threadID, pick, messageID, uid, replyText, pick);
...
module.exports.handleEvent = async function ({ api, event, Users }) {
  try {
    ...
    if (matched) {
      ...
      if (!q) return;
      const res  = await fetchWithRetry(`${simsim}/baby?text=${encodeURIComponent(q)}&sender=${encodeURIComponent(senderName)}`);
      if (isApiError(res)) return;
      const pick = unwrap(res).reply || "...";
      // API's reply already personalizes with "@sender ..." — don't double-prefix the name.
      return sendPlain(api, event.threadID, pick, event.messageID, senderID, q, pick);
    }

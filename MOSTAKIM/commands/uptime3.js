...
module.exports = {
  ...
  onStart: async function ({ api, event, usersData, threadsData, commandsData }) {
    ...
    try { userCount  = global.data?.allUserID?.length   || 0; } catch {}
    try { groupCount = global.data?.allThreadID?.length || 0; } catch {}
...
function getDisk() {
  try {
    // /home/runner is the actual workspace disk in Replit
    const out   = execSync("df -P /home/runner | tail -1").toString().trim().split(/\s+/);

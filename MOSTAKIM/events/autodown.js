...
const PATTERNS = {
  ...
  facebook:/(?:https?:\/\/)?(?:www\.|m\.)?(?:facebook\.com\/(?:watch|reel|videos|share\/[a-z]|permalink|permalink\.php)|fb\.(?:watch|me))\/?[\w\/\-?=%&.]*/i,
};
...
function detectUrl(body) {
  ...
  if (PATTERNS.facebook.test(body)) {
    const m = body.match(/(?:https?:\/\/)?(?:www\.|m\.)?(?:facebook\.com\/(?:watch|reel|videos|share\/[a-z]|permalink|permalink\.php)[^\s]*|fb\.(?:watch|me)\/[^\s]+)/i);
...
async function downloadYouTube(url, outPath) {
  ...
  try {
    ...
    if (sz < 1024) throw new Error("ytdl-core output too small");
    return { title, quality: best.qualityLabel || "SD" };
  } catch {}

  // Fallback: mostakim-media-downloaders (nayan package — different endpoint)
  try {
    ...
    const videoUrl = result.data.video_hd || result.data.video;
    if (!videoUrl) throw new Error("ytdown no video URL");
    ...
    return { title: result.data.title || "YouTube Video", quality: result.data.quality ? result.data.quality + "p" : "SD" };
  } catch {}

  // Last resort: direct nayan API (returns low/high video URLs)
  ...
  const d = res.data;
  if (!d.status || !d.data) throw new Error("nayan API failed");
  const videoUrl = d.data.high || d.data.low;
  if (!videoUrl) throw new Error("nayan no URL");
  const vr = await axios.get(videoUrl, { responseType: "arraybuffer", timeout: 90000, headers: { "User-Agent": "Mozilla/5.0" } });
  fs.writeFileSync(outPath, Buffer.from(vr.data));
  return { title: d.data.title || "YouTube Video", quality: "SD" };
}

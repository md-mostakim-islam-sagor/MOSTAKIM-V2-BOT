...
const { sendTelegram, setupTelegramConsole, classifyLoginError, escHtml, codeBlock, getBotName } = require("./utils/telegram");
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        if (loginError) {
            ...
            const _time    = new Date().toLocaleString("en-US", { timeZone: _tz, hour12: true, month: "2-digit", day: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" });
            const tgMsg    =
                `${escHtml(label)}\n` +
                `🤖 <b>Bot:</b> ${getBotName()}\n` +
                `⏰ <b>Time:</b> ${escHtml(_time)}\n\n` +
                `📋 <b>Detail:</b>\n` +
                codeBlock(errStr.slice(0, 2500));
            logger(errStr, `ERROR`);
        ...
        try {
            ...
            const banner   = figlet.textSync('MOSTAKIM', {
                font:             'ANSI Shadow',
                horizontalLayout: 'default',
                verticalLayout:   'default',
            });
            console.log('\n' + gradient(['#a855f7', '#ec4899', '#f97316']).multiline(banner) + '\n');
        } catch (_) {}
        ...
        function listenerCallback(error, message) {
            if (error && error !== null && !(error.type === 'ready' && error.error === null)) {
                ...
                if (tg.enable && tg.botToken && tg.chatId) {
                    ...
                    sendTelegram(
                        `${emoji} <b>${escHtml(label)}</b>\n` +
                        `🤖 <b>Bot:</b> ${getBotName()}\n` +
                        `⏰ <b>Time:</b> ${escHtml(_time)}\n` +
                        `🔖 <b>Type:</b> ${escHtml(errType)}\n\n` +
                        `📋 <b>Detail:</b>\n` +
                        codeBlock(errStr.slice(0, 2500)),
        ...
        if (tgNotify.enable && tgNotify.botToken && tgNotify.chatId) {
            ...
            const _adminUIDs = global.config.ADMINBOT || [];
            const _adminName = _adminUIDs.length > 0
                ? (_adminUIDs.map(uid => {
                    const name = global.data && global.data.userName && global.data.userName.get(String(uid));
                    return name || `UID:${uid}`;
                }).join(', '))
                : 'N/A';
            const _cmdCount  = global.client.commands ? global.client.commands.size : 0;
            const _evtCount  = global.client.events   ? global.client.events.size   : 0;
            const _tgMsg     = `👽 "${_botName}" is now online!\n⚙️ ADMIN: ${_adminName}\n🤖 UID: ${_botUID}\n🔧 Prefix: ${_prefix}\n⏰ Time: ${_time}\n🌐 Platform: ${_platform}\n📦 Command: ${_cmdCount} | Event: ${_evtCount}`;
        ...
        function writeBotStats() {
            try {
                ...
                const stats = {
                    ...
                    updatedAt:   Date.now(),
                    threads,
                    threadIDs:   global.data ? (global.data.allThreadID || []) : [],
                    banned,
                };
        ...
        const BROADCAST_QUEUE = join(__dirname, 'MAIN/bot-broadcast-queue.json');
        setInterval(async () => {
            try {
                if (!existsSync(BROADCAST_QUEUE)) return;
                const queue = JSON.parse(readFileSync(BROADCAST_QUEUE, 'utf-8'));
                const pending = queue.filter(j => j.status === 'pending');
                if (!pending.length) return;
                for (const job of pending) {
                    job.status = 'processing';
                    writeFileSync(BROADCAST_QUEUE, JSON.stringify(queue, null, 2));
                    for (const threadID of job.threads) {
                        try {
                            const text = job.mode === 1
                                ? `📢 Notice from bot admin:\n\n${job.message}`
                                : job.message;
                            await new Promise(r => setTimeout(r, job.delay || 1000));
                            await new Promise((resolve) => {
                                global.client.api.sendMessage(text, threadID, () => resolve());
                            });
                            job.sent = (job.sent || 0) + 1;
                        } catch (_) { job.failed = (job.failed || 0) + 1; }
                    }
                    job.status = 'done';
